...
public class UpdateSecondaryContact extends Fragment {
    ...
    List<PrimaryContactDB> primaryContactDBList;
    List<SecondaryContactDB> secondaryContactDBList;
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        view = inflater.inflate(R.layout.fragment_update_secondary_contact, container, false);
        setRecyclerView();
        return view;
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        primaryContactViewModel.getPrimaryContact().observe(getActivity(), new Observer<List<PrimaryContactDB>>() {
            ...
            public void onChanged(List<PrimaryContactDB> primaryContactDBS) {
                primaryContactDBList = primaryContactDBS;
            }
        });
        ...
        secondaryContactsViewModel.getAllSecondaryContacts().observe(getActivity(), new Observer<List<SecondaryContactDB>>() {
            ...
            public void onChanged(List<SecondaryContactDB> secondaryContactDBS) {
                ...
                totalContactsAdded = secondaryContactDBS.size();
        ...
        addSecondaryContact.setOnClickListener(view1 -> {
            if (totalContactsAdded < 5) {
                ...
                alertDialog = alertDialogBuilder.setContactError("You cannot add more then 5 contact",
    ...
    private void contactPicked(Intent data) {
        ...
        try {
            ...
            boolean notInPrimaryContact = true;
            boolean notInSecondaryContact = true;
            for (PrimaryContactDB i : primaryContactDBList) {
                if (contactName.equals(i.getContactName())) {

                    AlertDialog alertDialog = alertDialogBuilder.setContactError(
                            "This contact already exist in the Primary contacts",
                            "Contact Already Exists!");
                    alertDialog.show();
                    notInPrimaryContact = false;
                    break;
                }
            ...
            for (SecondaryContactDB i : secondaryContactDBList){
                if (contactName.equals(i.getContactName())){
                    AlertDialog alertDialog = alertDialogBuilder.setContactError(
                            "This contact already exist in the Secondary contacts",
                            "Contact Already Exists!");
                    alertDialog.show();
                    notInSecondaryContact = false;
                    break;
                }
            }
            if (notInPrimaryContact && notInSecondaryContact){
                totalContactsAdded++;
                secondaryContactsViewModel.insert(new SecondaryContactDB(contactName, phoneNumber));
                reference.child("Secondary").child(contactName).setValue(phoneNumber);
            }
    ...
    private void setRecyclerView() {
        ...
        recyclerView = view.findViewById(R.id.updateSecondaryContacts);
        ...
        new ItemTouchHelper(new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.RIGHT) {
            ...
            public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
                SecondaryContactDB secondaryContactDB = adapter.getContact(viewHolder.getAdapterPosition());
                reference.child("Secondary").child(secondaryContactDB.getContactName()).removeValue();
                secondaryContactsViewModel.delete(secondaryContactDB);
                totalContactsAdded--;
            }
